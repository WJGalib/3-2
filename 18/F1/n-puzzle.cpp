...
bool manhattan = true, verbose = false;
...
class Board {
    ...
    int get_h_cost() {
        if (h_cost == -1) h_cost = manhattan? manhattan_distance() : hamming_distance();
        return h_cost; 
    };
...
void Board::a_star_search() {
    ...
    while (search->get_h_cost() != 0) {
        search = pq.top();
        c_list.push_back(search);
        pq.pop();
        if (verbose) {
            search->print();
            cout << "g=" << search->get_g_cost() << ", h=" << search->get_h_cost() << endl;
        };
        if (search->get_left_neighbour() && find(c_list.begin(), c_list.end(), search->get_left_neighbour()) == c_list.end())
            pq.push(search->get_left_neighbour());
        if (search->get_right_neighbour() && find(c_list.begin(), c_list.end(), search->get_right_neighbour()) == c_list.end())
            pq.push(search->get_right_neighbour());
        if (search->get_top_neighbour() && find(c_list.begin(), c_list.end(), search->get_top_neighbour()) == c_list.end())
            pq.push(search->get_top_neighbour());
        if (search->get_bottom_neighbour() && find(c_list.begin(), c_list.end(), search->get_bottom_neighbour()) == c_list.end())
    ...
    cout << "Maximum number of moves = " << moves-1 << endl;
    while (!path.empty()) path.top()->print(), path.pop();
...
int main (int argc, char** argv) {
    ...
    if (argc > 1) {
        if (!strcasecmp(argv[1], "h")) manhattan = false;
        else  if (!strcasecmp(argv[1], "m")) manhattan = true;
        else cout << "Invalid arguments, defaulting to Manhattan" << endl;
        if (argc > 2) {
            if (!strcasecmp(argv[2], "v")) verbose = true;
            else cout << "Unrecognised arguments, ignoring" << endl;
        };
    };
