...
inline duration<double> get_time_now(high_resolution_clock::time_point& timer) {
    return duration_cast<duration<double>>(high_resolution_clock::now() - timer);
};

double Graph::maxcut_GRASP (int max_itr, constr_algo algo, bool print_steps=false, bool print_part=false) {
    double w_ = -INFINITY, w_ls = 0, ls_time = 0;
    set<int> S, _S;
    int ls_itr = 0;
    ...
    for (int i=0; i<max_itr; i++) {
        ...
        ls_time += get_time_now(timer).count();
    ...
    if (print_steps) cout << ls_itr << "," << (int)w_ls << "," << max_itr << "," << ls_time;
    return w_;
};
...
void Graph::print_maxcut_GRASP_steps (int max_itr, string input_file, int rand_itr = 10) {
    ...
    duration<double> greedy_time = get_time_now(timer);
    duration<double> semi_greedy_time = duration<double>::zero(), random_time = duration<double>::zero();
    for (int i=0; i<rand_itr; i++) {
        timer = high_resolution_clock::now();
        w_semi_greedy += semi_greedy_maxcut (S, _S); S.clear(), _S.clear();
        semi_greedy_time += get_time_now(timer);
        timer = high_resolution_clock::now();
        w_random += random_maxcut (S, _S); S.clear(), _S.clear();
        random_time += get_time_now(timer);
    };
    w_semi_greedy /= rand_itr, w_random /= rand_itr, semi_greedy_time /= rand_itr, random_time /= rand_itr;
    cout << input_file << "," << nV << "," << nE << "," 
         << (int)w_random << "," << w_greedy << "," << (int)w_semi_greedy << ",";
    timer = high_resolution_clock::now();
    cout <<  maxcut_GRASP (max_itr, randomised, true) << ",randomised," << random_time.count() << "," << get_time_now(timer).count() << endl;
    cout << input_file << "," << nV << "," << nE << "," 
         << (int)w_random << "," << w_greedy << "," << (int)w_semi_greedy << ",";    
    timer = high_resolution_clock::now();
    cout << maxcut_GRASP (max_itr, greedy, true) << ",simple_greedy," << semi_greedy_time.count() << "," << get_time_now(timer).count() << endl;
    cout << input_file << "," << nV << "," << nE << "," 
         << (int)w_random << "," << w_greedy << "," << (int)w_semi_greedy << ",";
    timer = high_resolution_clock::now();
    cout << maxcut_GRASP (max_itr, semi_greedy, true) << ",semi-greedy," << greedy_time.count() << "," << get_time_now(timer).count() << endl;
};
